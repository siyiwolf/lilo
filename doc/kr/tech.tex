...
\usepackage[bookmarks,%
        pdftitle={Linux Loader (LILO) 기술적 개요},%
...
{\bf 이 문서는 현재의\footnote{작성당시 1998년을 기준으로 함} 리눅스와 \LILO\ (version 21)의 상태가 부분적으로 반영되었습니다.}
...
첫번째 부트섹터가 0x96A00 으로 이동하면 화면에 "L"이 표시되고,
두번째 부트로더가 시작되기 전에 화면에 "I"가 표시됩니다.
만약 두번째 부트로더를 불러오는 과정에서 에러가 발생했다면,
첫번째 "L"이 출력되고, 두자리의 16진수로 이루어진 에러코드가 출력됩니다.
이것이 일시적인 문제가 아니라면 에러코드는 끝없이 반복될 것이며,
빌드시 {\tt NO1STDIAG} 옵션을 줌으로써 에러코드가 표시되는것을 끌 수 있습니다.
