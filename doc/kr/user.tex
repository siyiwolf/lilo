...
이 배포판은 이전 버전과 호환되지 않는 부분들을 기록한 \path{INCOMPAT} 파일을 제공합니다. 
...
\begin{itemize}
  \item \emphasize{당황하지 마세요.} 뭔가가 작동하지 않더라도,
  ...
  \item \emphasize{항상} 커널이나 \LILO\ 의 어떤 부분(설정 파일 포함)이 변경되어 진다면 \path{/sbin/lilo} 를 실행 하세요. 변경 유무가 확실치 않을 때에도 실행 하세요. 너무 자주 \path{/sbin/lilo} 를 실행 할 수는 없습니다.
  ...
  \item If performing a destructive upgrade and/or erasing your Linux
    ...
    MBR.
  \item 설치 스크립트를 신뢰하지 마세요. 언제나 부팅전에 스크립트가 생성한 \path{/etc/lilo.conf} 를 확인하세요.
  \item Don't trust setup scripts. Always verify the \path{/etc/lilo.conf}
    they create before booting.
  \item 용량이 큰 디스크를 사용한다면 이에 따른 불편함에 대비하여 \craw{linear} 옵션을 사용해야 합니다.
...
\begin{description}
  \item[맵 인스톨러] 는 \LILO\ 와 관련된 모든 파일들을 적절한 장소에 배치하고 부팅 시간에 필요한 위치 데이터 정보를 기록하는 리눅스 시스템 프로그램입니다. 이 프로그램은 보통 \path{/sbin/lilo} 에 위치해 있습니다. \LILO\ 가 인지하는 시스템의 어느 한 부분이라도 변경된다면 정보를 갱신하기 위해 이 프로그램을 실행해야 합니다, e.g. 새로운 커널을 설치한 뒤 
  ...
  \item[다양한 파일들] 은 \LILO\ 가 부팅 시간에 필요로 하는 데이터를 포함하고 있습니다, e.g. 부트 로더. 이 파일들은 보통 \path{/boot} 에 위치해 있습니다. 가장 중요한 파일은 부트 로더 (아래에 나오는)와 맵 인스톨러가 커널의 위치를 기록해 놓은 맵 파일(\path{/boot/map}) 입니다. \footnote{\LILO\ 는 파일 시스템을 어떻게 읽는지 모릅니다. 대신 맵 인스톨러가 커널로부터 파일들 (e.g. 커널 이미지)의 물리적 위치를 알아내 해당 정보를 기록합니다. 이는 \LILO\ 가 리눅스에서 지원되는 대부분의 파일 시스템과 동작할 수 있도록 해줍니다.} 또 다른 중요 파일은 설정 파일로 보통 \path{/etc/lilo.conf} 에 위치합니다.
...
\begin{description}
  \item[룻 파일 시스템 파티션] 은 다음 두 가지 이유로 중요합니다. 첫째, \LILO\ 는 때때로 커널에게 어디서 룻 파일 시스템 파티션을 찾을지 알려야 합니다. 둘째, 룻 파일 시스템 파티션은 흔히 여러 항목(부트 섹터, \path{/boot} 디렉토리, 커널)들을 위해 사용되는 편리한 장소 입니다. 
  ...
  \item[부트 섹터] 는 \LILO\ 부트 로더의 첫번째 부분을 포함합니다. 이 부분은 더 큰 두번째 단계의 로더를 로드합니다. 두 로더는 보통 \path{/boot/boot.b} 파일에 저장됩니다.
  ...
  \item[커널] 은 부트 로더로 부터 로드되고 시작됩니다. 커널은 보통 룻 디렉토리 또는 \path{/boot} 에 위치하고 있습니다.
...
전체 디스크는 리눅스에서 하나의 디바이스로 보입니다. (e.g. \path{/dev/fd0})
...
\LILO\ 는 비슷한 부트 섹터를 사용하지만 디스크 파라메터 부분은 없습니다. 부트 섹터를 살펴보지 않는 Minix, Ext2 또는 비슷한 파일 시스템에서는 문제가 없습니다. 그러나 \LILO\ 를 MS-DOS 파일 시스템의 부트 섹터에 둔다면 MS-DOS 에서 \LILO\ 에 접근할 수 없게 만들게 됩니다.
...
하드 디스크는 플로피 디스크보다 좀 더 복잡하게 구성됩니다. 하드 디스크는 파티션이라 불리는 여러 데이터 영역을 가지고 있습니다. 최대 4개까지 만들 수 있는 프라이머리 파티션은 MS-DOS 하드 디스크에 존재 할 수 있습니다. 더 많은 파티션이 필요하다면 하나의 프라이머리 파티션을 여러개의 논리 파티션을 가지는 익스텐디드 파티션으로 사용합니다.
...
각 하드 디스크의 첫번째 섹터는 파티션 테이블을 가지고 있습니다. 그리고 익스텐디드 파티션과 \emphasize{각각의} 논리 파티션도 파티션 테이블을 가지고 있습니다.
...
전체 디스크는 \path{/dev/hda}, \path{/dev/hdb}, \path{/dev/sda} 등등으로 접근 할 수 있습니다. 프라이머리 파티션은 \path{/dev/hda1 $\ldots$/dev/hda4} 입니다.
...
이 하드 디스크는 두 개의 프라이머리 파티션과 두개의 논리 파티션을 가진 하나의 익스텐디드 파티션을 가지고 있습니다. 이 파티션들은 \path{/dev/hd5 $\ldots$} 으로 접근할 수 있습니다.
...
논리 파티션의 파티션 테이블이 일부 디바이스의 첫번째 블럭에 있다면 접근이 불가할 수 있습니다. 반면 메인 파티션 테이블, 모든 부트 섹터 그리고 익스텐디드 파티션 테이블은 가능합니다.
...
파티션 테이블은 파티션 부트 섹터에 저장됩니다. 보통, 전체 디스크의 유일한 하나의 파티션 부트 섹터가 부트 섹터로 사용됩니다. 이는 흔히 마스터 부트 레코드(MBR)라고 불리어 집니다. 구조는 다음과 같습니다.
...
\LILO\ 부트 섹터는 파티션 부트 섹터로서 사용되도록 디자인 되었습니다. (I.e. 파티션 테이블을 위한 공간이 있습니다.) 그러므로, \LILO\ 부트 섹터는 다음 위치에 저장되어 질 수 있습니다.
...
\begin{itemize}
  \item 리눅스 플로피 디스크의 부트 섹터. (\path{/dev/fd0}, $\ldots$)
  \item boot sector of a Linux floppy disk. (\path{/dev/fd0}, $\ldots$)
  \item 하드 디스크의 MBR. (\path{/dev/hda}, \path{/dev/sda}, $\ldots$)
  \item MBR of the first hard disk. (\path{/dev/hda}, \path{/dev/sda}, $\ldots$)
  \item 첫번째 하드 디스크 프라이머리 리눅스 파일 시스템 파티션의 부트 섹터. (\path{/dev/hds1}, $\ldots$)
  \item boot sector of a primary Linux file system partition on the first hard
    disk. (\path{/dev/hda1}, $\ldots$)
  \item 첫번째 하드 디스크 익스텐디드 파티션의 부트섹터. (\path{/dev/hds1}, $\ldots$)\footnote{대부분의 FDISK 타입 프로그램은 익스텐디드 파티션으로 부터 부팅 되어진다고 예상하지 않으며 익스텐디드 파티션이 활성화 되는것을 거부합니다. \LILO\ 는 위 제한 사항이 없는 간단한 프로그램 (\name{activate})과 함께 있습니다. 리눅스 \name{fdisk} 는 또한 익스텐디드 파티션이 활성화 되는 것을 도와줍니다.}
...
\LILO\ 부트 섹터는 다음 위치에 \emphasize{저장되어 질 수 없습니다.}
It \emphasize{can't} be stored at any of the following locations:

\begin{itemize}
  \item 비-리눅스 플로피 디스크의 부트섹터 혹은 프라이머리 파티션.
  \item boot sector of a non-Linux floppy disk or primary partition.
  \item 리눅스 스왑 파티션.
  \item a Linux swap partition.
  \item 익스텐디드 파티션의 논리 파티션 부트 섹터.%
  \footnote{\LILO\ 는 \raw{-b} 옵션이나 \craw{boot} 변수를 사용하여 파티션에 놓일 수 있게 강제될 수 있습니다. 그러나 마스터 부트 레코드로서 실행되는 소수의 프로그램만 논리 파티션으로 부터 부팅되는 것을 돕습니다.}
...
\footnote{\LILO\ can be forced to put the boot sector on such a
 ...
 support booting from a logical partition.}
  \item 두번째 하드 디스크. (백업 설치를 위한 것이 아니라면, 현재 첫번째 디스크가 사용 불가 하거나 제거 되었다면, 혹은 다른 부트 로더가 사용된다면 두번째 하드 디스크는 다른 드라이버로 부터 부트 섹터를 로딩할 수 있습니다.
...
\LILO\ 는 유효하지 않은 곳에 부트 섹터를 놓는 시도를 감시하지만 이것에만 의지 하지 않도록 합니다. 
